package org.example.visitor;


import _2010._02.tyviseadus_1_10.Aktinimi;
import _2010._02.tyviseadus_1_10.AlampunktType;
import _2010._02.tyviseadus_1_10.AlgatusmargeType;
import _2010._02.tyviseadus_1_10.AlljaotisType;
import _2010._02.tyviseadus_1_10.Allkiri;
import _2010._02.tyviseadus_1_10.AllkirjastajaType;
import _2010._02.tyviseadus_1_10.AnyURIID;
import _2010._02.tyviseadus_1_10.AvaldamismargeType;
import _2010._02.tyviseadus_1_10.Base64BinaryID;
import _2010._02.tyviseadus_1_10.ChangeType;
import _2010._02.tyviseadus_1_10.ChangestringID;
import _2010._02.tyviseadus_1_10.DateID;
import _2010._02.tyviseadus_1_10.GYearID;
import _2010._02.tyviseadus_1_10.Isik;
import _2010._02.tyviseadus_1_10.IsikType;
import _2010._02.tyviseadus_1_10.JaguType;
import _2010._02.tyviseadus_1_10.JalgiMuudatusi;
import _2010._02.tyviseadus_1_10.JaotisType;
import _2010._02.tyviseadus_1_10.JuurdepaasPiirangType;
import _2010._02.tyviseadus_1_10.KehtivusType;
import _2010._02.tyviseadus_1_10.Kommentaar;
import _2010._02.tyviseadus_1_10.LegaaldefinitsioonType;
import _2010._02.tyviseadus_1_10.LisaOsaType;
import _2010._02.tyviseadus_1_10.LisaType;
import _2010._02.tyviseadus_1_10.LisaViideType;
import _2010._02.tyviseadus_1_10.Loend;
import _2010._02.tyviseadus_1_10.LoigeType;
import _2010._02.tyviseadus_1_10.MetaandmedType;
import _2010._02.tyviseadus_1_10.MuuJaotisType;
import _2010._02.tyviseadus_1_10.MuutmismargeType;
import _2010._02.tyviseadus_1_10.NonNegativeIntegerID;
import _2010._02.tyviseadus_1_10.Normtehnmarkus;
import _2010._02.tyviseadus_1_10.Oigusakt;
import _2010._02.tyviseadus_1_10.OsaType;
import _2010._02.tyviseadus_1_10.ParagrahvType;
import _2010._02.tyviseadus_1_10.PealkiriType;
import _2010._02.tyviseadus_1_10.PeatykkType;
import _2010._02.tyviseadus_1_10.PiirangLoppType;
import _2010._02.tyviseadus_1_10.PiirangPikendamineType;
import _2010._02.tyviseadus_1_10.PreambulType;
import _2010._02.tyviseadus_1_10.PunktType;
import _2010._02.tyviseadus_1_10.SeletuskiriLisaType;
import _2010._02.tyviseadus_1_10.SeletuskiriOsaType;
import _2010._02.tyviseadus_1_10.SeletuskiriType;
import _2010._02.tyviseadus_1_10.SisuType;
import _2010._02.tyviseadus_1_10.StringID;
import _2010._02.tyviseadus_1_10.TabelType;
import _2010._02.tyviseadus_1_10.TavatekstID;
import _2010._02.tyviseadus_1_10.TekstType;
import _2010._02.tyviseadus_1_10.TimeID;
import _2010._02.tyviseadus_1_10.TypeID;
import _2010._02.tyviseadus_1_10.Valjakuulutamine;
import _2010._02.tyviseadus_1_10.ValjakuulutatudType;
import _2010._02.tyviseadus_1_10.VastuvoetudType;
import _2010._02.tyviseadus_1_10.VeaparandusType;
import _2010._02.tyviseadus_1_10.ViideType;

import jakarta.annotation.Generated;


@Generated("Generated by jaxb-visitor")
public interface Visitor<R, E extends Throwable> {


    R visit(Aktinimi aBean)
            throws E
            ;

    R visit(AlampunktType aBean)
            throws E
            ;

    R visit(AlampunktType.AlampunktNr aBean)
            throws E
            ;

    R visit(AlgatusmargeType aBean)
            throws E
            ;

    R visit(AlgatusmargeType.Esindaja aBean)
            throws E
            ;

    R visit(AlljaotisType aBean)
            throws E
            ;

    R visit(AlljaotisType.AlljaotisNr aBean)
            throws E
            ;

    R visit(Allkiri aBean)
            throws E
            ;

    R visit(AllkirjastajaType aBean)
            throws E
            ;

    R visit(AnyURIID aBean)
            throws E
            ;

    R visit(AvaldamismargeType aBean)
            throws E
            ;

    R visit(AvaldamismargeType.RTosa aBean)
            throws E
            ;

    R visit(Base64BinaryID aBean)
            throws E
            ;

    R visit(ChangeType aBean)
            throws E
            ;

    R visit(ChangestringID aBean)
            throws E
            ;

    R visit(DateID aBean)
            throws E
            ;

    R visit(GYearID aBean)
            throws E
            ;

    R visit(Isik aBean)
            throws E
            ;

    R visit(IsikType aBean)
            throws E
            ;

    R visit(JaguType aBean)
            throws E
            ;

    R visit(JaguType.JaguNr aBean)
            throws E
            ;

    R visit(JalgiMuudatusi aBean)
            throws E
            ;

    R visit(JaotisType aBean)
            throws E
            ;

    R visit(JaotisType.JaotisNr aBean)
            throws E
            ;

    R visit(JuurdepaasPiirangType aBean)
            throws E
            ;

    R visit(KehtivusType aBean)
            throws E
            ;

    R visit(Kommentaar aBean)
            throws E
            ;

    R visit(LegaaldefinitsioonType aBean)
            throws E
            ;

    R visit(LisaOsaType aBean)
            throws E
            ;

    R visit(LisaType aBean)
            throws E
            ;

    R visit(LisaType.LisaMarge aBean)
            throws E
            ;

    R visit(LisaType.LisaPealkiri aBean)
            throws E
            ;

    R visit(LisaType.LisaSisu aBean)
            throws E
            ;

    R visit(LisaViideType aBean)
            throws E
            ;

    R visit(LisaViideType.LisaPealkiri aBean)
            throws E
            ;

    R visit(LisaViideType.LisaViit aBean)
            throws E
            ;

    R visit(Loend aBean)
            throws E
            ;

    R visit(LoigeType aBean)
            throws E
            ;

    R visit(LoigeType.LoigeNr aBean)
            throws E
            ;

    R visit(MetaandmedType aBean)
            throws E
            ;

    R visit(MetaandmedType.EmsNr aBean)
            throws E
            ;

    R visit(MetaandmedType.Versioon aBean)
            throws E
            ;

    R visit(MetaandmedType.VersiooniMuutus aBean)
            throws E
            ;

    R visit(MuuJaotisType aBean)
            throws E
            ;

    R visit(MuuJaotisType.JaotusNumber aBean)
            throws E
            ;

    R visit(MuutmismargeType aBean)
            throws E
            ;

    R visit(NonNegativeIntegerID aBean)
            throws E
            ;

    R visit(Normtehnmarkus aBean)
            throws E
            ;

    R visit(Oigusakt aBean)
            throws E
            ;

    R visit(OsaType aBean)
            throws E
            ;

    R visit(OsaType.OsaNr aBean)
            throws E
            ;

    R visit(ParagrahvType aBean)
            throws E
            ;

    R visit(ParagrahvType.ParagrahvNr aBean)
            throws E
            ;

    R visit(PealkiriType aBean)
            throws E
            ;

    R visit(PeatykkType aBean)
            throws E
            ;

    R visit(PeatykkType.PeatykkNr aBean)
            throws E
            ;

    R visit(PiirangLoppType aBean)
            throws E
            ;

    R visit(PiirangPikendamineType aBean)
            throws E
            ;

    R visit(PreambulType aBean)
            throws E
            ;

    R visit(PreambulType.Viide aBean)
            throws E
            ;

    R visit(PunktType aBean)
            throws E
            ;

    R visit(PunktType.PunktNr aBean)
            throws E
            ;

    R visit(SeletuskiriLisaType aBean)
            throws E
            ;

    R visit(SeletuskiriLisaType.SeletuskirjaLisaSisu aBean)
            throws E
            ;

    R visit(SeletuskiriOsaType aBean)
            throws E
            ;

    R visit(SeletuskiriOsaType.SeletuskirjaOsaSisu aBean)
            throws E
            ;

    R visit(SeletuskiriType aBean)
            throws E
            ;

    R visit(SisuType aBean)
            throws E
            ;

    R visit(StringID aBean)
            throws E
            ;

    R visit(TabelType aBean)
            throws E
            ;

    R visit(TabelType.Tr aBean)
            throws E
            ;

    R visit(TabelType.Tr.Tc aBean)
            throws E
            ;

    R visit(TavatekstID aBean)
            throws E
            ;

    R visit(TekstType aBean)
            throws E
            ;

    R visit(TekstType.Fail aBean)
            throws E
            ;

    R visit(TekstType.Viide aBean)
            throws E
            ;

    R visit(TimeID aBean)
            throws E
            ;

    R visit(TypeID aBean)
            throws E
            ;

    R visit(Valjakuulutamine aBean)
            throws E
            ;

    R visit(ValjakuulutatudType aBean)
            throws E
            ;

    R visit(VastuvoetudType aBean)
            throws E
            ;

    R visit(VeaparandusType aBean)
            throws E
            ;

    R visit(ViideType aBean)
            throws E
            ;

    R visit(ViideType.ViideAkt aBean)
            throws E
            ;

    R visit(ViideType.ViideURID aBean)
            throws E
            ;

}
